...
const AimInCompleted = ({ newAim }) => {
  ...
  const [currentAimToEdit, setCurrentAimToEdit] = useState(null);
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await aimAPI.getTaskInComplete(params);
    ...
    <div className="container mt-4">
      ...
      {dataInCompleted.map((item, index) => (
        ...
        >
          ...
          {currentAimToEdit && currentAimToEdit.id === item.id && (
            <AimToEditForm
              data={currentAimToEdit}
              close={() => setCurrentAimToEdit(null)}
              fetchData={() => fetchData()}
            />
          )}
